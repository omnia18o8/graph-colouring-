...
int main() {
    ...
    // Step 2: Generate permutation - in place random permutation of vertices
    ...
    std::unordered_map<int, int> list_sizes = getlistsizes(GraphInitialisation::V, GraphInitialisation::Delta, position, vertices);
    ...
    // Step 5a: Prepare order and adjacency only from the conflict graph vertices
    ...
    // Step 5b: Colour the conflict graph using greedy algorithm
    ...
    std::iota(full_palette.begin(), full_palette.end(), 1); // Full palette from 1 to Delta
    ...
    // Step 6: Colour non-conflict vertices
